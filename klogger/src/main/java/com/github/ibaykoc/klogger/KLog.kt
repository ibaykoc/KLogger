...
class KLog {
    ...
    companion object {
        ...
        fun error(clazz: KClass<*>, message: String, stackLevelUp: Int = defaultStackLevelUp) {
            if (isDebug)
                System.out.println(
                    "KLOG:E [${clazz.simpleName}${if (includeLineLocation) ", ${getFileLocation(
                        stackLevelUp
                    )}" else ""}]: $message"
                )
        }
        ...
        fun log(clazz: KClass<*>, message: String, stackLevelUp: Int = defaultStackLevelUp) {
            if (isDebug)
                System.out.println(
                    "KLOG:D [${clazz.simpleName}${if (includeLineLocation) ", ${getFileLocation(
                        stackLevelUp
                    )}" else ""}]: $message"
                )
        }
        ...
        private fun getFileLocation(stackLevelUp: Int): String =
            "(${getFileName(stackLevelUp)}:${getLineNumber(stackLevelUp)})"
